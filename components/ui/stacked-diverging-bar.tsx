...
interface DivergingSegment {
  key: string; // Series name, e.g., 'Completed', 'Remaining'
  data: number | null;
}

interface ChartCategoryData {
  key: string; // Day label, e.g., 'Mon', 'Tue'
  data: DivergingSegment[];
}

// Generate daily progress data for the last 7 days
const generateDailyProgressData = (): ChartCategoryData[] => {
  ...
  return days.map((day, index) => {
    // Calculate progress (0-100) for each day
    const completed = Math.floor(Math.random() * 60) + 20; // 20-80% completed
    ...
    return {
      ...
      data: [
        ...
        { key: 'Remaining', data: -remaining },
      ],
    };
  });
};

const binnedDateDataRaw: ChartCategoryData[] = generateDailyProgressData();

// Validate and prepare chart data
const validatedBinnedDateData = binnedDateDataRaw.map(category => ({
  ...category,
...
const chartColors = ['#EE4094', '#FAE5F6']; // Pink shades for progress
...
interface DivergingProgressCardProps {
  title?: string;
  totalProgress?: number;
}

function DivergingProgressCard({ title = "Progress Tracking", totalProgress = 0 }: DivergingProgressCardProps): JSX.Element {
  return (
    <div className="flex flex-col pt-4 pb-4 bg-white rounded-xl shadow-sm w-full overflow-hidden">
      <h3 className="text-3xl text-left p-7 pt-6 pb-8 font-bold text-gray-900">
        {title}
      </h3>
      <div className="flex-grow px-4 h-[200px]">
        <StackedBarChart
          id="diverging-progress-chart"
          height={200}
          data={validatedBinnedDateData}
          yAxis={
            <LinearYAxis
              tickSeries={<LinearYAxisTickSeries fill="#9A9AAF" />}
            />
          }
          xAxis={
            <LinearXAxis
          ...
          gridlines={
            <GridlineSeries
              line={<Gridline strokeColor="rgba(126, 126, 143, 0.15)" />}
            />
          }
        />
      </div>
      <div className="flex flex-col pl-8 pr-8 pt-8 font-mono divide-y divide-gray-200">
        {metrics.map(metric => (
          <motion.div
            key={metric.id}
          ...
          >
            <div className="flex flex-row gap-2 items-center text-base w-1/2 text-gray-500">
              {metric.iconSvg}
            ...
            <div className="flex gap-2 w-1/2 justify-end items-center">
              <span className="font-semibold text-xl text-gray-900">{metric.value}</span>
              {metric.trendIconSvg}
            </div>
          </motion.div>
        ))}
...
export default DivergingProgressCard;
